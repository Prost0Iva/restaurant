import { priceUpd } from './_sandwiches_options_navig.js';
    ...
    options.forEach((option) => {
        option.addEventListener('click', async function () {
            ...
            if (data.settings[page].multiple == true) {
                if (page == 'sauce') {
                    if (
                        !option.classList.contains('modal-option-active') &&
                        document.querySelectorAll('.modal-option-active').length < 3
                    ) {
                        option.classList.add('modal-option-active');
                ...
                options.forEach((v) => {
                    v.classList.remove('modal-option-active');
                });
                option.classList.add('modal-option-active');
