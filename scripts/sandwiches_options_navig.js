...
function changePage(page) {
    ...
    if (page == 0 || page == navig[0].id.slice(6)) {
        modal_previous.classList.add('hidden');
    } else modal_previous.classList.remove('hidden');
    if (page == navig.length - 1 || page == navig[navig.length - 1].id.slice(6)) {
        modal_next.classList.add('hidden');
    } else modal_next.classList.remove('hidden');
}
...
async function optionPageFill(page) {
    ...
    if (page == 'finishs') {
        ...
        finish.innerHTML = `<div class = "option-img-frame">
        ...
        modal_foot.innerHTML = `<div id = "modal-value">
                <p>КОЛИЧЕСТВО</p>
        ...
        modal_foot.innerHTML = `<p id="modal-total-price">Итого: 0 руб.</p>`;
    }
    priceUpd();
...
function finishButtons() {
    document.querySelector('.modal-val-remove').addEventListener('click', function () {
        if (Number(document.querySelector('.modal-val-indicator').textContent) > 1) {
            document.querySelector('.modal-val-indicator').textContent =
                Number(document.querySelector('.modal-val-indicator').textContent) - 1;
        }
    });
    ...
    document.querySelector('.modal-add-to-cart').addEventListener('click', function () {
        ...
        if (!matched) {
            cart.positions.push({
                ...
                price: Number(
                    modal_foot.querySelector('#modal-total-price').textContent.trim().split(' ')[1]
                ),
...
document.addEventListener('productListFilled', function () {
    ...
    products.forEach((product) => {
        ...
        hyper_text.addEventListener('click', function () {
            openModal(desc);
        });
    });
});
